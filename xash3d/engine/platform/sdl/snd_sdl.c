...
qboolean SNDDMA_Init( void *hInst )
{
	...
	desired.samples  = 1024; // Increased buffer size
	desired.channels = 2;
	...
	// Optionally check if obtained.samples matches desired.samples
	if( obtained.samples != desired.samples )
	{
		Con_Printf( "Warning: Obtained buffer size %d differs from desired %d.\n", obtained.samples, desired.samples );
	}
	...
	Con_Printf( "Using SDL audio driver: %s @ %d Hz, buffer size %d samples\n", SDL_GetCurrentAudioDriver( ), obtained.freq, obtained.samples );
